...
// الترتيب الذي طلبتِه: يبدأ بـ a18 كواجهة، ثم داخل المعرض يبدأ بـ a6 ثم البقية
const customGalleryOrder = [
  ...
  '/a16.jpg',
  '/a17.jpg',
  '/a18.jpg' // أضفتها في الآخر أيضاً ليتصفحوها كاملة
];
...
export default function InteractiveGallery() {
  ...
  return (
    // تغيير الخلفية إلى الأبيض الناعم (Slate-50) بدلاً من الأسود
    <section className="py-24 bg-slate-50 min-h-screen flex items-center justify-center overflow-hidden selection:bg-[#580303] selection:text-white">
      <div className="max-w-6xl w-full px-6 flex flex-col md:flex-row items-center gap-16 relative z-10">
        ...
        {/* الجانب النصي - ألوان واضحة وفخمة */}
        ...
        {/* الإطار التفاعلي - ألوان فاتحة وحديثة */}
        <div className="relative md:w-2/3 w-full h-[550px] bg-white rounded-[50px] border border-slate-200 flex items-center justify-center p-4 md:p-6 group shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
          ...
          {/* خلفية منقطة خفيفة جداً بالرمادي */}
            ...
            {!isOpen ? (
              ...
              <div className="relative w-full h-full group cursor-pointer" onClick={() => setIsOpen(true)}>
                <img 
                  ...
                  alt="IPFC Experience"
                />
                {/* تم إلغاء الـ Gradient الغامق واستبداله بلمسة خفيفة جداً */}
              ...
              <div className="relative w-full h-full animate-in fade-in zoom-in-95 duration-700">
                ...
                <button 
                  ...
                  className="absolute top-8 right-8 bg-white/90 backdrop-blur-md text-slate-400 hover:text-[#580303] p-2 rounded-xl transition-colors shadow-sm"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>

                <div className="absolute bottom-8 left-8 flex items-center gap-4 bg-white/80 backdrop-blur-md px-4 py-2 rounded-full border border-slate-100 shadow-sm">
          ...
          {/* خطوط جمالية بلون المركز الأحمر الخفيف */}
          <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-20">
            <path d="M50,50 Q300,20 550,150" stroke="#580303" strokeWidth="0.5" fill="none" strokeDasharray="8,8" />
          </svg>
        </div>
      </div>
    </section>
